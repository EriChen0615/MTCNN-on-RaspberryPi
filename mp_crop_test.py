...
def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor):
    ...
    # first stage
    if minsize==20: scales = [0.128, 0.08, 0.148, 0.4, 0.1]
    elif minsize==12: scales = [1.0,0.9,0.8,0.5,0.4]
...
def detect_process(qin,qout):
    ...
    qout.put(('Initialized',None,None,None)) # signal main process that initialization has completed

    while True:
        if qin.empty():
            continue

        frame_gray, time_stamp, offset = qin.get()
        ...
        if offset[0]==0 and offset[1]==0:
            ...
            boundingboxes, points = detect_face(img_matlab, 12, PNet, RNet, ONet, threshold, False, 0.6)
        qout.put((boundingboxes,points,time_stamp,offset))
        #toc()
...
def update_bbox(total_boxes,new_box,coord):
    ...
    new_box = np.array([new_box]) # make dimension match for concatenatation
    for i in range(total_boxes.shape[0]):
        if _contain(co_box,total_box[i]) and iou(new_box,total_box[i])>0.6:
            if len(new_box) != 0:
                total_boxes = np.concatenate((total_boxes[:i],new_box,total_boxes[i+1:]))
            else:
                total_boxes = np.concatenate((total_boxes[:i],total_boxes[i+1:]))
            return total_boxes
    #print("new_box shape:",new_box.shape)
    #print("total_box shape:",total_boxes.shape)
...
def main():
    ...
    boundingboxes = np.ndarray((0,5)) # dimension was 9
    crop_boxes = np.ndarray((0,5))
    ...
    while True:
        ...
        c_img,c_offset = gen_crop_img(img_gray,crop_boxes)
        current_time = timer()
        #sleep(0.01)# small delay to prevent overload
        input_queue.put((img_gray,current_time,(0,0,320,240)))
        for c_i,c_o in zip(c_img,c_offset):
            cv2.imshow('cropped',c_i)
            input_queue.put((c_i,current_time,c_o))
        
        if not output_queue.empty():
            _boundingboxes, _points, ts, _offset = output_queue.get()
            if _offset == (0,0,320,240):
                crop_boxes = _boundingboxes
                print('crop_boxes:',crop_boxes)
            elif ts-last_time>0:
                #print("Detection FPS = {0}".format(1.0/(ts-last_time)))
                _boundingboxes = offset_bboxes(_boundingboxes,_offset)
                for new_box in _boundingboxes:
                    print("newbox:",new_box)
                    update_bbox(boundingboxes,new_box,_offset)
                points = _points
                last_time = ts
